...
import { Badge, Button, TextInput, Toast } from 'flowbite-react'
    ...
    if (
      ...
      try {
        ...
        setToast({
          show: true,
          message:
            'Account deleted successfully. Redirecting to homepage now...',
          type: 'success',
        })
        setTimeout(() => {
          handleSignOut()
    ...
    <div className="dashboard mx-5 mb-20 min-h-full flex flex-row gap-1">
      ...
      <div className="container mx-auto pr-20">
        ...
        {toast.show && (
          <Toast>
            <div
              className={`inline-flex h-8 w-8 shrink-0 items-center justify-center rounded-lg bg-${toast.type === 'success' ? 'green' : 'red'}-100 text-${toast.type === 'success' ? 'green' : 'red'}-500 dark:bg-${toast.type === 'success' ? 'green' : 'red'}-800 dark:text-${toast.type === 'success' ? 'green' : 'red'}-200`}
            >
              {toast.type === 'success' ? (
        ...
        {Object.keys(editMode).map((field) => (
          <div
            key={field}
            className="flex items-center mb-4 bg-white shadow-md rounded-md p-4"
          >
        ...
        <div className="flex items-center mb-4 bg-white shadow-md rounded-md p-4">
          <div className="flex-grow">
            <Button onClick={handleDeleteAccount} size="xs" color="red">
              Delete Account
            </Button>
          </div>
        </div>
