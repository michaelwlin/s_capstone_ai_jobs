...
def login(username, password):
    ...
    if response.status_code == 200:
        ...
        print("/n/n",response_data,"/n/n/")
        user = response_data.get('user')
        user_id = response_data.get('userId')
        return user, user_id, response.cookies
    else:
...
def send_get_request(user_id, cookies):
    url = f"{base_url}/users/{user_id}"
    response = requests.get(url, cookies=cookies)
    return response, response.status_code


def send_get_logged_in_data_request(cookies):
    url = f"{base_url}/users/loggedInData"
    response = requests.get(url, cookies=cookies)
...
assert post_status_code == 401, f"Expected 401 Unauthorized, but received {post_status_code}"
